...
bool sodaDistanceFieldFont::loadFont(string _filename, bool _bFullCharacterSet, float _simplifyAmt, int _dpi) {
	...
	if ( read( sdff_filename ) ) {
		...
		for (int i = 0 ; i < nCharacters; i++){
			...
			int width  = cps[i].width + 4; // JONS: allow for antialiasing
			int height = bitmap.rows + 4;
...
void sodaDistanceFieldFont::createStringMesh(string c, float x, float y, float scale, float maxWidth){
    ...
    float yAdvance = ( (lineHeight*1.5)*newLineDirection ) * scale;
    ...
    while(index < len){
        ...
        float charWidth = c[index] == ' ' || c[index] == '\n' ? 0.0 : ( cps[cy].setWidth * letterSpacing )  * scale;
        if (cy < nCharacters){ 			// full char set or not?
            if ( isspace(c[index]) ) lastWhiteSpace = index;
            if (c[index] == '\n' || ( maxWidth > 0 && X + charWidth > maxX ) ) {
                if ( lastWhiteSpace < 0 ) return;
                int trim = index - lastWhiteSpace;
                index = lastWhiteSpace;
                line.resize(line.length()-trim);
                lines.push_back(line);
                Y += yAdvance;
                X = x ; //reset X Pos back to zero
                line.clear();
            }else if (c[index] == ' ') {
                int cy = (int)'o' - NUM_CHARACTER_TO_START;
                line.push_back(c[index]);
                X += ( cps[cy].setWidth * letterSpacing * spaceSize ) * scale;
            } else if(cy > -1){
                line.push_back(c[index]);
                X += charWidth;
    ...
    for ( int i = 0; i < lines.size(); i++ ) {
        ...
        index = 0;
        while(index < len){
            int cy = (unsigned char)line[index] - NUM_CHARACTER_TO_START;
            float charWidth = line[index] == ' ' || line[index] == '\n' ? 0.0 : ( cps[cy].setWidth * letterSpacing )  * scale;
            if (cy < nCharacters){ 			// full char set or not?
              if (line[index] == ' ') {
...
ofMesh & sodaDistanceFieldFont::getStringMesh(string c, float x, float y, float scale){
	stringQuads.clear();
	createStringMesh(c,x,y,scale);
	return stringQuads;
}
