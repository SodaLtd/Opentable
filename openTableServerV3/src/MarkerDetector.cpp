...
void MarkerDetector::processFrame(const BGRAVideoFrame& frame)
{
    ...
    {
        m_transformations.push_back(m_markers[i].transformation);
    }
    unlock();
}
...
bool MarkerDetector::findMarkers(const BGRAVideoFrame& frame, std::vector<ARMarker>& detectedMarkers)
{
...
(
    ...
    std::vector<ARMarker>& detectedMarkers
) 
{
    std::vector<cv::Point>  approxCurve;
    std::vector<ARMarker>     possibleMarkers;
    ...
    { 
        const ARMarker& m1 = possibleMarkers[i];

        //calculate the average distance of each corner to the nearest corner of the other marker candidate
        for (size_t j=i+1;j<possibleMarkers.size();j++)
        {
            const ARMarker& m2 = possibleMarkers[j];
...
void MarkerDetector::recognizeMarkers(const cv::Mat& grayscale, std::vector<ARMarker>& detectedMarkers)
{
    std::vector<ARMarker> goodMarkers;
...
void MarkerDetector::estimatePosition(std::vector<ARMarker>& detectedMarkers)
{
    for (size_t i=0; i<detectedMarkers.size(); i++)
    {					
        ARMarker& m = detectedMarkers[i];
...
int MarkerDetector::markerAt( float x, float y ) {
    int marker_id = -1;
    lock();
    
    for ( int i = 0; i < m_markers.size(); i++ ) {
        if ( m_markers[ i ].inside(x, y)) {
            marker_id = m_markers[ i ].id;
            break;
        }
    }
    unlock();
