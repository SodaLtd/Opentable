...
openTableSerialDevice::openTableSerialDevice() {
...
//
//
bool openTableSerialDevice::setup( int device ) {
...
bool openTableSerialDevice::setup( string port ) {
	m_connection.setup( port, 9600 );
	if ( !m_connection.isInitialized() ) {
		ofLogError("openTableSerialDevice::setup") << "unable to initialise connection";
	}
	return m_connection.isInitialized();
}
void openTableSerialDevice::update() {
	if ( m_connection.isInitialized() ) {
...
map< string, string > openTableSerialDevice::listDevices( int min_port, int max_port ) {
